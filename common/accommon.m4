...
dnl so remove it.
	...
	with_rexx="oorexx"
		...
		REXX_TARGET="ooRexx"
		extra_rexx_libs="-lrexx"
		extra_incdirs="/usr/local/include/ooRexx /usr/include/ooRexx"
		extra_libdirs="/usr/local/lib/ooRexx /usr/lib/ooRexx"
		case "$target" in
			*linux*)
			extra_rexx_defines="-DLINUX"
			;;
...
mh_found_rpm="not found"
for mh_path in $1; do
   find $mh_path/SOURCES -type d 1>&5 2>&5
   if test $? -eq 0 ; then
      mh_found_rpm="$mh_path"
      break
   fi
done
RPMTOPDIR="$mh_found_rpm"
AC_SUBST(RPMTOPDIR)
AC_MSG_RESULT($mh_found_rpm)
])

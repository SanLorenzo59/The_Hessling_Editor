...
static char RCSid[] = "$Id: parser.c,v 1.31 2006/01/29 08:03:33 mark Exp $";
      ...
      switch( curr->column )
      {
            ...
            || ( state == STATE_COMMENT && *ptr == '\0' ) )
            {
               for ( i = pos; i < len; i++ )
               {
                  ...
                  &&   i-vcol < THE_MAX_SCREEN_WIDTH )
                  {
                     ...
                     /*
                     scurr->highlight_type[i-vcol] = THE_SYNTAX_COMMENT;
                      * Don't set the highlight_type array here, set it for the
                      * remainder of the line below
                      */
                  }
                  if ( work[i] != ' ' )
                  {
               ...
               /*
                * Set the remainder of the line to COMMENT for QUERY SYNELEM
                */
               i = pos - vcol; /* This is the location of the start of the incomplete string */
               memset( scurr->highlight_type+i, THE_SYNTAX_COMMENT, THE_MAX_SCREEN_WIDTH - i );
            }
            break;
         case MAX_INT:  /* 'firstnonblank' */
            for ( i = 0; i < len; i++ )
         ...
         default:    /* specific column */
            ...
            || ( state == STATE_COMMENT && *ptr == '\0' ) )
            {
               ...
               /*
                * Set the remainder of the line to INCOMPLETE for QUERY SYNELEM
                */
               i = pos - vcol; /* This is the location of the start of the incomplete string */
               memset( scurr->highlight_type+i, THE_SYNTAX_COMMENT, THE_MAX_SCREEN_WIDTH - i );
            }
   ...
   for ( i = 0; i < screen[scrno].rows[WINDOW_FILEAREA]; i++ )
   {
      ...
      while( ptr )
      {
         ...
         if ( type == STATE_START_TAG )
         {
            ...
            &&   start_line_comment < pos )
            {
                ...
                * Set the remainder of the line to COMMENT for QUERY SYNELEM
                */
               j = start_line_comment - vcol; /* This is the location of the start of the comment */
               memset( scurr->highlight_type+j, THE_SYNTAX_COMMENT, THE_MAX_SCREEN_WIDTH - j );
               comment_start_pos = -1;
               type = 0;
               break;
            }
         ...
         else
         {
             ...
             * Don't do this for HTML tags, because the end tag is the same as
             * a normal tag.
             */
            if ( first
            && !( fd->parser->have_markup_tag
                  &&   CURRENT_VIEW->syntax_headers & HEADER_MARKUP ) )
            {
               for ( k = 0; k < i; k++ )
               {
                  ...
                  if ( scurr->is_cursor_line
                  &&   scurr->is_cursor_line_filearea_different )
                     comment_colour1 = merge_curline_colour( fd->attr+ATTR_CURSORLINE, fd->ecolour+ECOLOUR_COMMENTS );
                  else if ( scurr1->is_current_line )
                  ...
                  if ( scurr1->line_type == LINE_LINE )
                  {
                      ...
                      * Set the remainder of the line to COMMENT for QUERY SYNELEM
                      */
                     memset( scurr1->highlight_type, THE_SYNTAX_COMMENT, THE_MAX_SCREEN_WIDTH );
                  }
               }
               comment_start_pos = 0;
            }
            ...
            if ( comment_start_pos != -1 )
            {
               ...
               for ( j = comment_start_pos; j < pos+len_end; j++ )
               {
                  ...
                  {
                     scurr->highlighting[j-vcol] = comment_colour;
                     scurr->highlight_type[j-vcol] = THE_SYNTAX_COMMENT;
                  }
                  if (work[j] != ' ')
                  {
                     work[j] = ' ';
      ...
      &&  type == STATE_START_TAG)
      {
         ...
         if ( comment_start_pos == 0 )
            ...
            j = comment_start_pos - vcol; /* This is the location of the start of the comment */
         memset( scurr->highlight_type+j, THE_SYNTAX_COMMENT, THE_MAX_SCREEN_WIDTH - j );
         comment_start_pos = 0;
      }
      else
...
/***********************************************************************/
{
   ...
   ||  check_double_quote)
   {
            ...
            case STATE_START:
               ...
               &&   check_single_quote )
               {
                  ...
                  &&   off < THE_MAX_SCREEN_WIDTH )
                  {
                     ...
                     scurr->highlight_type[off] = THE_SYNTAX_STRING;
                  }
                  if ( work[i] != ' ' )
                  {
                     number_blanks++;
      ...
      if ( first_quote < len )
      {
          ...
          * Set the remainder of the line to INCOMPLETE for QUERY SYNELEM
          */
         i = first_quote - vcol; /* This is the location of the start of the incomplete string */
         if ( i < THE_MAX_SCREEN_WIDTH )
            memset( scurr->highlight_type+i, THE_SYNTAX_INCOMPLETESTRING, THE_MAX_SCREEN_WIDTH - i );
      }
   }
   TRACE_RETURN();
   return RC_OK;
               ...
               for (j=pos;j<i;j++)
               {
                  if (work[j] != ' ')
                  {
                     if (j >= vcol && j-vcol < THE_MAX_SCREEN_WIDTH)
                     {
                        ...
                        scurr->highlight_type[j-vcol] = highlight_type;
                     }
                     number_blanks++;
                     work[j] = ' ';
                  }
...
/***********************************************************************/
{
   ...
   if ( scurr->is_cursor_line
   &&   scurr->is_cursor_line_filearea_different )
      header_colour = merge_curline_colour(fd->attr+ATTR_CURSORLINE,fd->ecolour+ECOLOUR_HEADER);
   else if (scurr->is_current_line)
            ...
            || (state == STATE_HEADER && *ptr == '\0'))
            {
               for (i=pos;i<len;i++)
               {
                  if (i >= vcol && i-vcol < THE_MAX_SCREEN_WIDTH)
                  {
                     ...
                     scurr->highlight_type[i-vcol] = THE_SYNTAX_HEADER;
                  }
                  if (work[i] != ' ')
                  {
                     number_blanks++;
               ...
               if (*ptr == '\0')
               {
                  for (k=curr->header_column-1;k<len;k++)
                  {
                     if (k >= vcol && k-vcol < THE_MAX_SCREEN_WIDTH)
                     {
                        ...
                        scurr->highlight_type[k-vcol] = THE_SYNTAX_HEADER;
                     }
                     if (work[k] != ' ')
                     {
                        number_blanks++;
               ...
               if (*ptr == '\0')
               {
                  for (k=fd->parser->label_column-1;k>-1;k--)
                  {
                     if (k >= vcol && k-vcol < THE_MAX_SCREEN_WIDTH)
                     {
                        ...
                        scurr->highlight_type[k-vcol] = THE_SYNTAX_LABEL;
                     }
                     if (work[k] != ' ')
                     {
                        number_blanks++;
   ...
   for (i=0;i<len;i++)
   {
      ...
      ||  work[i] == '}')
      {
         if (i >= vcol && off < THE_MAX_SCREEN_WIDTH)
         {
            ...
            scurr->highlight_type[off] = THE_SYNTAX_MATCH;
         }
         work[off] = ' ';
         number_blanks++;
      }
            ...
            for (j=0;j<re_len;j++,i++)
            {
               if ( work[i] != ' ' )
               {
                  if (i >= vcol && i-vcol < THE_MAX_SCREEN_WIDTH)
                  {
                     ...
                     scurr->highlight_type[i-vcol] = THE_SYNTAX_POSTCOMPARE;
                  }
                  work[i] = ' ';
                  number_blanks++;
               }
               ...
               if ( re_len > 0 )
               {
                  for ( j = 0 ; j < re_len; j++, i++ )
                  {
                     if ( i >= vcol && i-vcol < THE_MAX_SCREEN_WIDTH )
                     {
                        ...
                        scurr->highlight_type[i-vcol] = THE_SYNTAX_NUMBER;
                     }
                     if ( work[i] != ' ' )
                     {
                        work[i] = ' ';
            ...
            if ( find_keyword( fd, work + i, re_len, &alt ) )
            {
               ...
               for ( j = 0; j < re_len; j++, i++ )
               {
                  if (i >= vcol && i-vcol < THE_MAX_SCREEN_WIDTH)
                  {
                     ...
                     scurr->highlight_type[i-vcol] = THE_SYNTAX_KEYWORD;
                  }
                  work[i] = ' ';
                  number_blanks++;
               }
                     ...
                     if ( find_keyword( fd, (CHARTYPE *)"call", 4, &alt ) )
                     {
                        ...
                        for ( j = 0; j < 4; j++ )
                        {
                           if ( word_start+j >= vcol && (word_start+j)-vcol < THE_MAX_SCREEN_WIDTH )
                           {
                              ...
                              scurr->highlight_type[(word_start+j)-vcol] = THE_SYNTAX_KEYWORD;
                           }
         ...
         if ( find_function( fd, work + i, function_len, &alt ) )
         {
            ...
            for ( j = 0; j < function_len; j++, i++ )
            {
               if ( i >= vcol && i-vcol < THE_MAX_SCREEN_WIDTH )
               {
                  ...
                  scurr->highlight_type[i-vcol] = THE_SYNTAX_FUNCTION;
               }
               work[i] = ' ';
               number_blanks++;
            }
               ...
               if ((re_len = find_preprocessor(fd,word,keyword_length,&alt)))
               {
                  ...
                  for (j=0,k=keyword_start_pos;j<re_len;j++,k++)
                  {
                     if (k >= vcol && k-vcol < THE_MAX_SCREEN_WIDTH)
                     {
                        ...
                        scurr->highlight_type[k-vcol] = THE_SYNTAX_KEYWORD;
                     }
                     work[k] = ' ';
                     number_blanks++;
                  }
                  if (preprocessor_char_start_pos >= vcol && preprocessor_char_start_pos-vcol < THE_MAX_SCREEN_WIDTH)
                  {
                     ...
                     scurr->highlight_type[preprocessor_char_start_pos-vcol] = THE_SYNTAX_KEYWORD;
                  }
...
/***********************************************************************/
{
   ...
   for (i=0;i<THE_MAX_SCREEN_WIDTH;i++)
   {
      ...
      scurr->highlight_type[i] = THE_SYNTAX_NONE;
   }
   number_blanks = 0;
   /*
...
/***********************************************************************/
{
   ...
   else
   {
      ...
      for (i=0;i<rsrvd->length || i<THE_MAX_SCREEN_WIDTH;i++)
      {
...
/***********************************************************************/
{
   ...
   {
      scurr += row;
      syntax_element = scurr->highlight_type[col];
   }
   TRACE_RETURN();
